<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0">
    ...
    <xsl:template match="DataElement">
        ...
        <data-element>
            <names>
                <id>US-NCICB-CACORE-CADSR-<xsl:value-of select="publicID"/>-<xsl:value-of select="version"></xsl:value-of></id>
                <preferred><xsl:value-of select="longName"/></preferred>
                <all-names>
                    <name><xsl:value-of select="preferredName"/></name>
                    <name><xsl:value-of select="longName"/></name>
                </all-names>
            </names>
            <definition><xsl:value-of select="preferredDefinition"/></definition>
    ...
    <xsl:template match="NonenumeratedValueDomain">
        ...
        <values>
            <non-enumerated>
                <data-type><xsl:value-of select="datatypeName"/></data-type>
                <units></units>
            </non-enumerated>                
        </values>
    </xsl:template>
    ...
    <xsl:template match="PermissibleValue">
        ...
        <valid-value>
            ...
            <meaning><xsl:value-of select="valueMeaning/ValueMeaning/shortMeaning"/></meaning>
        </valid-value>
    </xsl:template>
